...
|| defined(CONFIG_MACH_M3) || defined(CONFIG_MACH_T0)
	...
	if ((tkey_i2c->firmware_ver < TK_FIRMWARE_VER) &&
		...
		tkey_i2c->module_ver = data[2];
#endif
		pr_debug("[TouchKey] firm ver = %d, module ver = %d\n",
...
static irqreturn_t touchkey_interrupt(int irq, void *dev_id)
{
	...
	if (keycode_type == 1)
		pr_debug("search key sensitivity = %d\n",
		       search_sensitivity);
	if (keycode_type == 2)
...
static ssize_t touch_version_write(struct device *dev,
				   ...
				   const char *buf, size_t size)
{
	pr_debug("[TouchKey] input data --> %s\n", buf);
...
void touchkey_update_func(struct work_struct *work)
{
	...
	char data[3];
	i2c_touchkey_read(tkey_i2c->client, KEYCODE_REG, data, 3);
	pr_debug("[%s] F/W version: 0x%x, Module version:0x%x\n",
	...
	enable_irq(tkey_i2c->irq);
	tkey_i2c->update_status = TK_UPDATE_FAIL;
	pr_debug("[TouchKey] touchkey_update failed\n");
	return;
}
...
#endif				/* CONFIG_TARGET_LOCALE_NA */
		pr_debug("[TouchKey] touchkey firmware update\n");
				 ...
				 size_t size)
{
	...
	if (ret != 1) {
		pr_debug("[TouchKey] %s, %d err\n",
			__func__, __LINE__);
		return size;
	}

	if (data != 1 && data != 2) {
		pr_debug("[TouchKey] %s wrong cmd %x\n",
			__func__, data);
		return size;
	}
...
#ifdef CONFIG_TARGET_LOCALE_NA
	if (tkey_i2c->module_ver < 8) {
		pr_debug("called %s data[12] =%d,data[13] = %d\n",
		       __func__, data[12], data[13]);
		menu_sensitivity = ((0x00FF & data[12]) << 8) | data[13];
	} else {
		pr_debug("called %s data[17] =%d\n", __func__,
		       data[17]);
		menu_sensitivity = data[17];
	}
#else
	pr_debug("called %s data[10] =%d,data[11] = %d\n", __func__,
	       data[10], data[11]);
...
#ifdef CONFIG_TARGET_LOCALE_NA
	if (tkey_i2c->module_ver < 8) {
		pr_debug("called %s data[8] =%d,data[9] = %d\n",
		       __func__, data[8], data[9]);
		back_sensitivity = ((0x00FF & data[8]) << 8) | data[9];
	} else {
		pr_debug("called %s data[13] =%d\n", __func__,
		       data[13]);
		back_sensitivity = data[13];
	}
#else
...
|| (defined(CONFIG_MACH_C1) && defined(CONFIG_TARGET_LOCALE_KOR))\
	...
	pr_debug("called %s data[12] = %d, data[13] =%d\n", __func__,
			data[12], data[13]);
	menu_sensitivity = ((0x00FF & data[12]) << 8) | data[13];
	pr_debug("called %s menu_sensitivity =%d\n", __func__,
			menu_sensitivity);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	pr_debug("called %s back_sensitivity =%d\n", __func__,
			back_sensitivity);
#else
	u8 data[10];
	int ret;
