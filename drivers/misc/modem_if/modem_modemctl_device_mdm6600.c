...
static int mdm6600_on(struct modem_ctl *mc)
{
	...
	gpio_set_value(mc->gpio_pda_active, 1);
#if defined(CONFIG_MACH_M0_CTC)
	/* Make power on event (PM8028 KPD_PWR_N) */
	gpio_set_value(mc->gpio_cp_on, 1);
	/* Disable PM8028 hard reset ( 7 sec timer ) */
	gpio_set_value(mc->gpio_reset_req_n, 1); 
	msleep(200);

	/* Set MDM RESIN_N to high for preventing MSMC1 drop(MDM reset) */
	gpio_set_value(mc->gpio_cp_reset, 1);
	msleep(400);
#else
	gpio_set_value(mc->gpio_cp_on, 1);
	...
	msleep(50);
#endif
	/* Release power key(PM8028 KPD_PWR_N) */
	gpio_set_value(mc->gpio_cp_on, 0);
	msleep(500);
...
#if defined(CONFIG_MACH_M0_CTC)
	gpio_set_value(mc->gpio_cp_on, 0);        // don't make POWER ON event
	gpio_set_value(mc->gpio_cp_reset, 1);     // don't make MDM reset
	gpio_set_value(mc->gpio_reset_req_n, 1);  // don't make PM8028 reset
	msleep(10);

	gpio_set_value(mc->gpio_cp_reset, 0);     // make MDM reset (start)
	msleep(10);

	gpio_set_value(mc->gpio_cp_on, 1);        // make POWER ON event
	msleep(190);

	gpio_set_value(mc->gpio_cp_reset, 1);     // complete MDM reset (In order to prevent MSMC1 drop, reset time is 200 ms)
	msleep(310);

	gpio_set_value(mc->gpio_cp_on, 0);        // complete POWER ON event ( stay for 500 ms )
	msleep(100);
#else
	gpio_set_value(mc->gpio_cp_reset, 0);
	gpio_set_value(mc->gpio_reset_req_n, 0);
